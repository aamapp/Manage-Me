  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      <div 
        ...
        className={`w-full px-4 py-3.5 bg-transparent border border-slate-200 rounded-xl font-bold text-sm text-slate-800 outline-none focus:ring-2 focus:ring-[#1a73e8] flex items-center justify-between cursor-pointer transition-all ${isOpen ? 'ring-2 ring-[#1a73e8] border-[#1a73e8]' : ''}`}
        style={{ fontFamily: "'Kohinoor Bangla', sans-serif" }}
      >
        <span className="text-sm truncate mr-2 text-slate-600">
          {value ? new Date(value).toLocaleDateString('bn-BD') : (!label ? (placeholder || '') : '')}
        </span>
