  ...
  return (
    <div className="space-y-6 w-full max-w-full overflow-hidden">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-800 tracking-tight">স্বাগতম, {user?.name.split(' ')[0]} 👋</h1>
          <p className="text-slate-500 text-xs font-medium mt-0.5">আপনার ড্যাশবোর্ড</p>
        </div>
      </div>

      {/* Stats Grid - Compact 2x2 Layout */}
      <div className="grid grid-cols-2 gap-3">
        {/* Primary Income Card - Now same size as others */}
        <StatCard 
          title="মোট আয়" 
          value={totalIncome} 
          isCurrency={true} 
      ...
      {/* Chart Section */}
      <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm w-full overflow-hidden">
        <div className="flex items-center justify-between mb-4">
          <div>
             <h3 className="font-bold text-slate-800 text-sm">মাসিক আয়</h3>
             <p className="text-[10px] text-slate-400 font-medium">গত ৬ মাস</p>
          </div>
          <button 
        ...
        <div className="h-48 w-full -ml-2">
          {!hasChartData ? (
            <div className="h-full w-full flex flex-col items-center justify-center text-slate-300 gap-2 border-2 border-dashed border-slate-100 rounded-2xl ml-2">
              ...
              <p className="text-[10px] text-center px-4">পেমেন্ট রেকর্ড থাকলে চার্ট দেখা যাবে</p>
            </div>
          ) : (
          ...
          {statusSummary.map((status) => (
            <div key={status.label}>
              <div className="flex justify-between items-center mb-1.5 text-xs">
                ...
                <span className="font-black text-slate-800 bg-slate-50 px-1.5 py-0.5 rounded text-[10px]">{status.count}</span>
              </div>
      ...
      <div className="pb-4">
        <div className="flex items-center justify-between mb-3 px-1">
          ...
          <button 
            ...
            className="text-indigo-600 text-xs font-bold bg-indigo-50 px-2.5 py-1 rounded-full hover:bg-indigo-100 transition-colors"
          >
            সব দেখুন
          </button>
        </div>
          ...
          <div className="space-y-3">
            {recentProjects.map(p => (
              <div 
                key={p.id} 
                onClick={() => navigate('/projects')}
                className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm active:scale-[0.98] transition-all flex items-center justify-between group"
              >
                <div className="flex-1 min-w-0 mr-4">
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="text-[9px] font-black uppercase tracking-wider text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded truncate max-w-[80px]">
                  ...
                  <p className="text-[10px] text-slate-500 truncate font-medium">{p.clientname}</p>
                </div>
