...
import { CURRENCY, PROJECT_STATUS_LABELS, PROJECT_TYPE_LABELS } from '../constants';
...
export const Dashboard: React.FC = () => {
  ...
  const completedProjects = projects.filter(p => p.status === 'Paid' || p.status === 'Delivered').length;
  const ongoingProjects = projects.filter(p => p.status === 'In Progress').length;

  const statusSummary = [
    { label: 'অপেক্ষমান', count: projects.filter(p => p.status === 'Pending').length, color: 'bg-amber-500' },
    { label: 'চলমান', count: projects.filter(p => p.status === 'In Progress').length, color: 'bg-blue-500' },
    { label: 'প্রেরিত', count: projects.filter(p => p.status === 'Delivered').length, color: 'bg-purple-500' },
    { label: 'পরিশোধিত', count: projects.filter(p => p.status === 'Paid').length, color: 'bg-emerald-500' },
  ];
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <div className="h-80 w-full flex items-center justify-center">
             <div className="text-slate-300 text-center">
                <p className="text-sm">পর্যাপ্ত আর্থিক তথ্য পাওয়া গেলে এখানে চার্ট প্রদর্শিত হবে।</p>
             </div>
          </div>
        </div>
