  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const monthlySum = incomeRecords.filter(record => {
        ...
        // Manual string parsing to avoid timezone offsets causing date shifts
        // Record date format is expected to be "YYYY-MM-DD"
        const parts = record.date.split('-');
        if (parts.length < 3) return false;
        
        const rYear = parseInt(parts[0], 10);
        const rMonth = parseInt(parts[1], 10); // 1-12
        
        // Compare Year and Month (convert 1-based month to 0-based index)
        return rYear === targetYear && (rMonth - 1) === targetMonthIndex;
