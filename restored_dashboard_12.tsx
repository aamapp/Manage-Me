...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6 w-full max-w-full overflow-hidden pt-2">
      ...
      {/* Chart Section */}
      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm w-full overflow-hidden">
        <div className="flex items-center justify-between mb-6">
          <div>
             <h3 className="font-bold text-slate-800 text-base">মাসিক আয়</h3>
             <p className="text-xs text-slate-400 font-medium mt-1">গত ৬ মাসের আয়ের হিসাব</p>
          </div>
          <button 
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
        <h3 className="font-bold text-slate-800 mb-5 text-base">প্রজেক্ট স্ট্যাটাস</h3>
        <div className="space-y-4">
          {statusSummary.map((status) => (
            <div key={status.label}>
              <div className="flex justify-between items-center mb-2 text-sm">
                <span className="text-slate-600 font-bold">{status.label}</span>
                <span className="font-black text-slate-800 bg-slate-50 px-2.5 py-1 rounded-lg text-xs">{status.count}</span>
              </div>
      ...
      <div className="pb-6">
        <div className="flex items-center justify-between mb-4 px-1">
          ...
          <button 
            ...
            className="text-indigo-600 text-xs font-bold bg-indigo-50 px-3 py-1.5 rounded-full hover:bg-indigo-100 transition-colors"
          >
            সব দেখুন
          </button>
        </div>
          ...
          <div className="space-y-4">
            {recentProjects.map(p => (
              <div 
                key={p.id} 
                onClick={() => navigate('/projects')}
                className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm active:scale-[0.98] transition-all flex items-center justify-between group"
              >
                <div className="flex-1 min-w-0 mr-4">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-[10px] font-black uppercase tracking-wider text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-md truncate max-w-[100px]">
                  ...
                  <h4 className="font-bold text-slate-800 text-base truncate mb-1">{p.name}</h4>
                  <p className="text-xs text-slate-500 truncate font-medium">{p.clientname}</p>
                </div>
                <div className="text-right whitespace-nowrap">
                  <p className="font-bold text-slate-800 text-base">{currency} {p.totalamount.toLocaleString('bn-BD')}</p>
                  <p className="text-[10px] text-slate-400 mt-1 font-bold bg-slate-50 px-2 py-1 rounded-md inline-block">{p.deadline}</p>
                </div>
              </div>
            ))}
          </div>
