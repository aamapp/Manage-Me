...
export const Dashboard: React.FC = () => {
  ...
  const totalIncome = incomeRecords.reduce((acc, curr) => acc + (Number(curr.amount) || 0), 0);
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      // Calculate target month/year relative to local time
      // Using day=1 avoids issues with different number of days in months
      const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
      
      const year = d.getFullYear();
      const monthIndex = d.getMonth(); // 0-11
      
      // Construct YYYY-MM string for reliable matching
      // Pad month with leading zero if needed (e.g., "2024-05")
      const monthKey = `${year}-${(monthIndex + 1).toString().padStart(2, '0')}`;
      
      const monthlySum = incomeRecords
        .filter(record => {
          if (!record.date) return false;
          // Check if the record date string starts with YYYY-MM
          // This works robustly for "YYYY-MM-DD" formatted strings
    ...
    <div className="space-y-5 w-full max-w-full overflow-hidden pt-2">
      ...
      <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm w-full overflow-hidden">
        ...
        <div className="h-52 w-full">
          {!hasChartData ? (
            <div className="h-full w-full flex flex-col items-center justify-center text-slate-300 gap-2 border-2 border-dashed border-slate-100 rounded-2xl">
