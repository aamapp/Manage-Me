...
export const Income: React.FC = () => {
  ...
  useEffect(() => {
    if (visibleLimit >= filteredPayments.length || isLoadingMore) return;
                ...
                <div>
                  ...
                  {isMobileScreen ? (
                    ...
                    <div className="relative">
                      <input
                        type="text"
                        value={String(newPayment.amount || "")}
                        onChange={(e) => {
                          const val = e.target.value;
                          const cleanVal = val.replace(/[^0-9.]/g, "");
                          setNewPayment({ ...newPayment, amount: Number(cleanVal) || 0 });
                        }}
                        placeholder="0"
                        className="w-full px-3 py-3 bg-slate-50 border border-slate-200 rounded-xl font-black text-xl text-emerald-600 outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition-colors pr-10 animate-in fade-in duration-150"
                      />
