                  ...
                  <div className="flex flex-col gap-1.5 text-left">
                    ...
                    {isMobileScreen ? (
                      ...
                      <div className="relative">
                        <input
                          type="text"
                          value={String(newProject.totalamount || "")}
                          onChange={(e) => {
                            const val = e.target.value;
                            const cleanVal = val.replace(/[^0-9.]/g, "");
                            setNewProject({ ...newProject, totalamount: cleanVal ? Number(cleanVal) : 0 });
                          }}
                          className={`w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-800 outline-none text-sm transition-all pr-10 min-h-[48px] ${showKeypad && activeAmountField === "total" ? "ring-2 ring-indigo-500 border-indigo-500" : "focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"}`}
                          style={{ fontFamily: "'Kohinoor Bangla', sans-serif" }}
                          placeholder="০"
                        />
                  ...
                  <div className="flex flex-col gap-1.5 text-left">
                    ...
                    {isMobileScreen ? (
                      ...
                      <div className="relative">
                        <input
                          type="text"
                          value={String(newProject.paidamount || "")}
                          onChange={(e) => {
                            const val = e.target.value;
                            const cleanVal = val.replace(/[^0-9.]/g, "");
                            setNewProject({ ...newProject, paidamount: cleanVal ? Number(cleanVal) : 0 });
                          }}
                          className={`w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-emerald-600 outline-none text-sm transition-all pr-10 min-h-[48px] ${showKeypad && activeAmountField === "paid" ? "ring-2 ring-indigo-500 border-indigo-500" : "focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"}`}
                          style={{ fontFamily: "'Kohinoor Bangla', sans-serif" }}
                          placeholder="০"
                        />
                        <div className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none">
