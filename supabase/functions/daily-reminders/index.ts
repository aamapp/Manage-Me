...
serve(async (req) => {
  ...
  try {
    ...
    for (const user of profiles || []) {
      ...
      const {
        data: { user: authUser },
      } = await supabaseClient.auth.admin.getUserById(user.id);
      const lastNotifiedAtStr = authUser?.user_metadata?.last_notified_at;
      const lastNotifiedAt = lastNotifiedAtStr
        ? new Date(lastNotifiedAtStr)
        : null;
      const now = new Date();
      ...
      if (
        ...
        console.log(
          `Skipping notification for user ${user.id} - Already notified recently.`,
        );
        continue;
      }
      ...
      for (const token of fcmTokens) {
        for (const notif of notificationsToSend) {
          ...
          if (fcmResponse.ok) {
            ...
            console.error(
              `Failed to send FCM to user ${user.id}:`,
