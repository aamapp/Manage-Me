...
serve(async (req) => {
  ...
  try {
    ...
    const PENDING_IMG =
      "https://qlmdoatgvovggvgzhwoy.supabase.co/storage/v1/object/public/notification-images/PENDING_IMG.png";
    const DUE_IMG =
      "https://qlmdoatgvovggvgzhwoy.supabase.co/storage/v1/object/public/notification-images/DUE_IMG.png";
    const INCOME_IMG =
      "https://qlmdoatgvovggvgzhwoy.supabase.co/storage/v1/object/public/notification-images/INCOME_IMG.png";
    const DUE_PERSON_IMG =
      "https://qlmdoatgvovggvgzhwoy.supabase.co/storage/v1/object/public/notification-images/DENA_PAWNA_IMG.png";
    const BACKUP_IMG =
      "https://qlmdoatgvovggvgzhwoy.supabase.co/storage/v1/object/public/notification-images/BACKUP_IMG.png"; // আপনার তৈরি করা ব্যাকআপ নির্দেশনা ইমেজটির URL এখানে বসাতে পারেন
        ...
        if (payload.type === "INSERT") {
          if (payload.record.status === "Completed") {
            ...
            notificationsToSend.push({
              ...
              imageUrl:
                (payload.record.status || "").toLowerCase() === "pending"
                  ? PENDING_IMG
                  : INCOME_IMG,
            });
          }
          ...
          if (payload.old_record) {
            ...
            dueChanged =
              payload.old_record.dueamount !== payload.record.dueamount;
          } else {
            statusChanged = true;
          }
    ...
    if (!userId || notificationsToSend.length === 0) {
      console.log("Missing required fields or no notifications to send.", {
        userId,
      });
      return new Response(
        JSON.stringify({
          message: "Missing required fields or no notifications",
        }),
        {
    ...
    if (fcmTokens.length === 0) {
      console.log(
        `No FCM token found for user ${userId}. Cannot send notification.`,
      );
      return new Response(
        JSON.stringify({ message: "User does not have an FCM token" }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 200,
        },
    ...
    if (!serviceAccountStr) {
      ...
      if (settingsError || !settingsData) {
        console.error(
          "Error fetching Firebase Service Account from app_settings:",
          settingsError,
        );
        throw new Error(
          "Firebase configuration not found in app_settings table or FIREBASE_SERVICE_ACCOUNT secret.",
        );
      }

      serviceAccountStr =
        typeof settingsData.firebase_service_account === "string"
    ...
    if (!serviceAccountStr) {
      throw new Error(
        "FIREBASE_SERVICE_ACCOUNT environment variable or app_settings is missing",
      );
    }
    ...
    return new Response(
      JSON.stringify({ success: true, responses: allResponses }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
      },
